...
//  val file
  ...
  val owners = scala.io.Source.fromFile(new File("OWNERS"))(Codec.UTF8).getLines().toList
  val dependencies = scala.io.Source.fromFile(new File("DEPDENDENCIES")).getLines().toList
  val cache = new collection.concurrent.TrieMap[String, Int]
